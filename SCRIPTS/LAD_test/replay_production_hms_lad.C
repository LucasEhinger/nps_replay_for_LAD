...
#include "/home/ehingerl/hallc/software/nps_replay_for_LAD/SCRIPTS/HMS/hms_shared_LAD.h"
void replay_production_hms_lad(Int_t RunNumber=0, Int_t MaxEvent=0) {
  ...
  const char* RunFileNamePattern = "nps_coin_%d.dat.0";  //Raw data file name pattern

  const char* ROOTFileNamePattern = "ROOTfiles/LAD_test/hms_replay_production_%d_%d.root";
  ...
  // TString summary_file = Form("REPORT_OUTPUT/LAD_test/summary_production_%d_%d.report",
	// 		      RunNumber, MaxEvent);
  // TString report_file  = Form("REPORT_OUTPUT/LAD_test/replay_hms_production_%d_%d.report",
	// 		      RunNumber, MaxEvent);
